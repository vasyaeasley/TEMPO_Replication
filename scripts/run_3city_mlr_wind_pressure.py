...
OUTPUT_DIR = BASE_DIR / "models" / "mlr_wind_pressure_3city"
...
print(
    "🌦️ STARTING 3-CITY MULTIPLE LINEAR REGRESSION (WIND SPEED + PRESSURE) 🌦️"
)
print("=" * 75)
...
for city_name, info in target_cities.items():
  print(f"\n🌍 BUILDING MLR (WIND + PRESSURE) FOR: {city_name.upper()}...")
  ...
  # Convert Units: Wind -> Knots, Pressure -> Millibars (hPa)
  wind_knots = (
      np.sqrt(X_stn[:, idx_u10] ** 2 + X_stn[:, idx_v10] ** 2) * 1.94384
  )
  press_mb = X_stn[:, idx_sp] / 100.0

  hours_per_day = 11
  n_days = len(y_stn) // hours_per_day

  d_no2, d_month, d_wind, d_press = [], [], [], []
  ...
  # 1. Fit Multiple Linear Regression (NO2 ~ Wind + Pressure)
  X_mlr = df_daily[["Wind", "Press"]].values
  ...
  coef_press = mlr_model.coef_[1]

  # 2. Calculate Partial Regression Residuals for Added-Variable Plots (AV-Plots)
  wind_res = (
      df_daily["Wind"]
      - LinearRegression()
      .fit(df_daily[["Press"]], df_daily["Wind"])
      .predict(df_daily[["Press"]])
  )
  no2_wind_res = (
      df_daily["NO2"]
      - LinearRegression()
      .fit(df_daily[["Press"]], df_daily["NO2"])
      .predict(df_daily[["Press"]])
  )
  ...
  press_res = (
      df_daily["Press"]
      - LinearRegression()
      .fit(df_daily[["Wind"]], df_daily["Press"])
      .predict(df_daily[["Wind"]])
  )
  no2_press_res = (
      df_daily["NO2"]
      - LinearRegression()
      .fit(df_daily[["Wind"]], df_daily["NO2"])
      .predict(df_daily[["Wind"]])
  )

  # 3. Fit Baselines for Comparison Table
  rmse_wind_only = np.sqrt(
      mean_squared_error(
          y_all,
          LinearRegression().fit(df_daily[["Wind"]], y_all).predict(df_daily[["Wind"]]),
      )
  )
  ...
  rmse_harm = np.sqrt(
      mean_squared_error(
          y_all, LinearRegression().fit(X_harm, y_all).predict(X_harm)
      )
  )
  r2_harm = r2_score(
      y_all, LinearRegression().fit(X_harm, y_all).predict(X_harm)
  ...
  # ==============================================================================
  # 4. RENDER 4-PANEL MLR DIAGNOSTIC GRID (CLEAN UNICODE TEXT)
  # ==============================================================================
  ...
  max_val = max(y_all.max(), preds_mlr.max()) * 1.05
  min_val = min(y_all.min(), preds_mlr.min())
  ax_tl.plot(
      [min_val, max_val],
      [min_val, max_val],
      "k--",
  ...
  res_txt = f"Mean = {np.mean(res_mlr):+.2f} ppb\nStd Dev = {res_std:.2f} ppb"
  ax_tr.text(
      0.05,
      0.88,
      res_txt,
  ...
  ax_bl.set_title(
      "Partial Regression: Wind Speed\n(controlling for Surface Pressure)",
      fontsize=13,
      fontweight="bold",
  )
  ax_bl.set_xlabel(
  ...
  # --- Panel 4 (Bottom Right): Partial Regression for Surface Pressure ---
  ax_br.scatter(press_res, no2_press_res, color="#c084fc", alpha=0.55, s=25)
  p_range = np.linspace(press_res.min(), press_res.max(), 100)
  ax_br.plot(p_range, coef_press * p_range, "k-", linewidth=2.5)

  ax_br.set_title(
      "Partial Regression: Surface Pressure\n(controlling for Wind Speed)",
      fontsize=13,
      fontweight="bold",
  )
  ax_br.set_xlabel(
      "Surface Pressure (residualized) [mb]", fontsize=11, fontweight="bold"
  )
  ax_br.set_ylabel(
      "NO₂ (residualized) [ppb]", fontsize=11, fontweight="bold"
  )
  ...
  fig.suptitle(
      "Multiple Linear Regression: NO₂ ~ Wind Speed + Surface"
      f" Pressure\n({city_name} Site 2023-2024)",
      fontsize=16,
      fontweight="bold",
  )
  plt.savefig(
      OUTPUT_DIR / f"{city_name.lower()}_mlr_4panel_diagnostic.jpg",
      dpi=300,
      bbox_inches="tight",
  )
  plt.close(fig)

  print(f"✅ Saved MLR 4-panel diagnostic chart for {city_name}!")
...
print("\n" + "=" * 80)
print("🏆 MULTIPLE LINEAR REGRESSION (WIND + PRESSURE) COMPARISON TABLE 🏆")
print("=" * 80)
