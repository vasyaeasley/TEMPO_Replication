...
print("🗺️ GENERATING GRIDDED CALIFORNIA MAP WITH HIGHWAYS & CLUSTERS 🗺️")
print("=" * 75)
...
for _, group in sites_df.groupby("Cluster"):
  tr, _ = train_test_split(group, test_size=0.40, random_state=42)
...
if HAS_GIS:
  ...
  # Exactly matches the paper's OpenStreetMap / Carto DB background style
  try:
    ctx.add_basemap(
        ax,
  ...
  # Format GPS coordinate labels instead of Web Mercator meters on the axes
  ax.set_axis_on()
...
ax.set_title(
    ...
    " Clusters\nCalifornia Gridded Road Network & Basin Coverage (60% Train /"
    " 40% Test)",
    fontsize=15,
    fontweight="bold",
    pad=18,
...
out_file = OUTPUT_DIR / "california_gridded_cluster_map_with_roads.png"
plt.savefig(out_file, dpi=300, bbox_inches="tight")
plt.close(fig)

print(f"✅ Saved Gridded Highway Cluster Map to: {out_file}")
print("=" * 75)
