...
OUTPUT_DIR = BASE_DIR / "models" / "mentor_table_slides"
...
print("📊 GENERATING PUBLICATION-GRADE MENTOR COMPARISON TABLE SLIDES 📊")
print("=" * 75)
...
for city_name, info in target_cities.items():
  print(f"\n🌍 EVALUATING BASELINES & RENDERING SLIDE FOR: {city_name.upper()}...")
  ...
  # 1. Harmonic Model (2 Harmonics)
  harm_model = LinearRegression().fit(X_train_harm, y_train)
  harm_preds = harm_model.predict(X_test_harm)
  ...
  # 2. Cosine Model (1 Harmonic)
  cos_model = LinearRegression().fit(X_train_harm[:, :2], y_train)
  cos_preds = cos_model.predict(X_test_harm[:, :2])
  ...
  print(
      f"   * Persistence -> RMSE: {rmse_pers:.2f} | MAE: {mae_pers:.2f} | R²:"
      f" {r2_pers:.3f}"
  )
  print(
      f"   * Harmonic    -> RMSE: {rmse_harm:.2f} | MAE: {mae_harm:.2f} | R²:"
      f" {r2_harm:.3f}"
  )
  print(
      f"   * Cosine      -> RMSE: {rmse_cos:.2f} | MAE: {mae_cos:.2f} | R²:"
      f" {r2_cos:.3f}"
  )

  # Determine best metrics to highlight in bold green
  best_rmse = min(rmse_pers, rmse_harm, rmse_cos)
  best_mae = min(mae_pers, mae_harm, mae_cos)
  best_r2 = max(r2_pers, r2_harm, r2_cos)
  ...
  ax.text(
      ...
      0.92,
      (
          "Model Comparison: Persistence, Cosine, and Harmonic\n"
  ...
  row_bgs = [
      ...
      "#fff8e1",
      "#e1f5fe",
  ]  # Green tint (Persist), Cream (Harmonic), Blue tint (Cosine)
  ...
  # Key Results Box at the bottom (matching rounded orange border style)
  key_results_text = (
      "Key Results:\n"
      f"• Persistence (RMSE={rmse_pers:.2f} ppb) captures strong day-to-day"
      " atmospheric weather autocorrelation.\n"
      f"• Harmonic (RMSE={rmse_harm:.2f} ppb) improves over basic Cosine by"
      " using 2 seasonal cycles (annual + semi-annual).\n"
      f"• Cosine (RMSE={rmse_cos:.2f} ppb) uses a single symmetrical annual"
      " cycle — simpler but misses seasonal asymmetry.\n"
      "• To definitively beat Persistence, our 20-feature XGBoost digital twin"
      " fuses DOY with live weather and traffic."
  )
  ...
  out_file = OUTPUT_DIR / f"{city_name.lower()}_model_comparison_table.png"
...
print("=" * 75)
print(
    f"🎯 ALL 3 MENTOR COMPARISON SLIDES SUCCESSFULLY GENERATED IN: {OUTPUT_DIR}"
)